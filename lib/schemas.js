...
const algs = ['PS256', 'PS384', 'PS512', 'RS256', 'RS384', 'RS512']
...
const READ_PERMISSION = Joi.object({
  ...
  type: Joi.string().valid('READ').required(),
  purpose: Joi.string().required(),
  kid: Joi.string().required() // the key of the service which is allowed to READ
})
...
// app -> operator
const RECIPIENTS_READ_REQUEST = Joi.object({
  ...JWT_DEFAULTS,
...
const RECIPIENT = Joi.object({
  header: Joi.object({
    ...
    alg: Joi.string().valid('RSA-OAEP').required()
  }).required(),
  encrypted_key: Joi.string().required()
}).required()

const RECIPIENTS_READ_RESPONSE = Joi.object({
  ...JWT_DEFAULTS,
...
// app -> operator
const RECIPIENTS_WRITE = Joi.object({
  ...JWT_DEFAULTS,
